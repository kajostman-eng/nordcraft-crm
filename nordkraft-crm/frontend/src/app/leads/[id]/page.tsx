...
import { documentsApi, leadsApi } from "@/lib/api";
...
export default function LeadDetailPage() {
  ...
  const { data: lead, isLoading: leadLoading } = useQuery<Lead>({
    ...
    queryFn: () => leadsApi.get(String(leadId)),
    enabled: !!leadId,
  });

  const { data: docs = [], isLoading: docsLoading } = useQuery<Document[]>({
    queryKey: ["documents", "lead", leadId],
    queryFn: () => documentsApi.list({ lead_id: String(leadId) }),
    enabled: !!leadId,
  });
  ...
  const uploadMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      setTitle("");
      setDescription("");
      await qc.invalidateQueries({ queryKey: ["documents", "lead", leadId] });
    },
  ...
  const createOfferMutation = useMutation({
    mutationFn: () =>
      offersApi.create({
        title: lead ? `Offer — ${lead.company_name || `${lead.first_name} ${lead.last_name}`}` : "Offer",
        lead_id: String(leadId),
        currency: "EUR",
      }),
    onSuccess: (offer: any) => {
  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="bg-nk-surface border border-nk-border rounded-xl p-4 mb-4">
        {leadLoading || !lead ? (
          <div className="text-[13px] text-nk-muted">Loading lead…</div>
        ) : (
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <div className="text-[12px] text-nk-muted mt-0.5">
                {lead.email} · {lead.company_name || "—"} · {lead.status.replace(/_/g, " ")}
              </div>
            </div>
            <div className="text-right">
              <div className="text-[11px] text-nk-muted uppercase tracking-[0.5px]">AI score</div>
              <div className="text-[18px] font-semibold">{lead.ai_score || 0}</div>
              <button
                onClick={() => createOfferMutation.mutate()}
                disabled={!lead || createOfferMutation.isPending}
      ...
      <div className="bg-nk-surface border border-nk-border rounded-xl overflow-hidden">
        ...
        <div className="divide-y divide-nk-border">
          ...
          {docs.length === 0 && !docsLoading && (
            <div className="px-4 py-10 text-center text-[12px] text-nk-muted">No documents for this lead yet.</div>
          )}
        </div>
      </div>
    </div>
