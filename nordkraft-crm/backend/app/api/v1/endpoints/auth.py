...
async def bootstrap_admin(
    ...
    Only allowed with the deployment bootstrap token and when there are no users yet.
    """
    if not settings.BOOTSTRAP_TOKEN or not bootstrap_token:
        raise HTTPException(status_code=403, detail="Invalid bootstrap token")
    if not compare_digest(bootstrap_token, settings.BOOTSTRAP_TOKEN):
        raise HTTPException(status_code=403, detail="Invalid bootstrap token")
