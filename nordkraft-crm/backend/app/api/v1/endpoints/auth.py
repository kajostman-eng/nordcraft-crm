...
def validate_bootstrap_token(token: str | None) -> None:
    if not settings.BOOTSTRAP_TOKEN:
        raise HTTPException(status_code=403, detail="Bootstrap token is not configured")
    if not token or not secrets.compare_digest(token, settings.BOOTSTRAP_TOKEN):
...
async def bootstrap_admin(
    ...
    """
    validate_bootstrap_token(x_bootstrap_token)
