...
_ASYNC_PG_TLS_MODES = {"allow", "prefer", "require", "verify-ca", "verify-full"}
_ASYNC_PG_CERT_QUERY_KEYS = {"sslrootcert", "sslcert", "sslkey", "sslpassword", "sslcrl"}
...
def _certificate_option_present(qs: dict[str, list[str]]) -> bool:
    return any(_query_value(qs, key) for key in _ASYNC_PG_CERT_QUERY_KEYS)
...
def _asyncpg_ssl_arg(qs: dict[str, list[str]], host: str):
    ...
    if sslmode in _ASYNC_PG_TLS_MODES:
        return sslmode

    if ssl in _ASYNC_PG_TLS_MODES:
        return ssl
