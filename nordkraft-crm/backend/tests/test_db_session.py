...
class AsyncpgEngineOptionsTests(unittest.TestCase):
    ...
    def test_verify_ca_sslmode_can_use_default_trust_store(self):
        url, kwargs = _asyncpg_engine_options(
            "postgresql+asyncpg://user:password@db.example.com:5432/app?sslmode=verify-ca"
        )
    ...
    def test_supabase_still_defaults_to_tls_required(self):
        url, kwargs = _asyncpg_engine_options(
            "postgresql+asyncpg://user:password@aws-0-us-east-1.pooler.supabase.com:5432/app"
        )
    ...
    def test_non_asyncpg_url_is_unchanged(self):
        url, kwargs = _asyncpg_engine_options("sqlite+aiosqlite:///tmp/test.db?sslmode=verify-full")

        self.assertEqual(url, "sqlite+aiosqlite:///tmp/test.db?sslmode=verify-full")
